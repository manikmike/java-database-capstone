...
public class DoctorService {
	...
	public List<String> getDoctorAvailability(Long doctorId, LocalDate date) {
		...
		for (Appointment a : appointments) {
			...
			bookedTimes.add(String.format("%02d:00-%02d:00", hour, hour+1));
		}
		List<String> availableTimes = doctorRepository.getReferenceById(doctorId).getAvailableTimes();
	...
	public int saveDoctor(Doctor doctor) {
		Doctor foundDoctor = doctorRepository.findByEmail(doctor.getEmail());
		try {
			if (foundDoctor != null) { // doctor exists => can't save new
				return -1;
			}
			else {
	...
	public int updateDoctor(Doctor doctor) {
		Doctor foundDoctor = doctorRepository.findByEmail(doctor.getEmail());
		try {
			if (foundDoctor == null) { // doctor doesn't exist => can't update
				return -1;
			}
			else {
				doctorRepository.save(doctor);
	...
	public int deleteDoctor(long doctorId) {
		try {
			...
			if (doctor == null) { // doctor doesn't exist => can't delete
				return -1;
			}
			else {
	...
	public ResponseEntity<Map<String, String>> validateDoctor(Login login) {
		...
		try {
			...
			Doctor foundDoctor = doctorRepository.findByEmail(email);
			if (foundDoctor == null) {
				response.put("message", "Invalid email address for doctor login");
				response.put("status", "404 Not Found");
				return ResponseEntity.status(HttpStatus.NOT_FOUND).body(response);
			}
			else if(!foundDoctor.getPassword().equals(login.getPassword())) {
				response.put("message", "Password incorrect for doctor login");
				response.put("status", "401 Unauthorized");
				return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(response);
			}
			else {
				response.put("message", "Login for doctor successful");
	...
	private List<Doctor> filterDoctorByTime(List<Doctor> doctors, String amOrPm) {
		...
		for (Doctor doc : doctors) {
			...
			for (String t : times) {
				int start = Integer.parseInt(t.substring(0, 2));
				if (amOrPm.equals("AM") && start < 12 || amOrPm.equals("PM") && start >= 12) {
	...
	// 18. **findDoctors Method**:
	//    - Finds all doctors.
	@Transactional
	public Map<String, Object> findDoctors() {
		Map<String, Object> doctorsMap = new HashMap<>();
		List<Doctor> doctors = doctorRepository.findAll();
		doctorsMap.put("doctors", doctors);
		return doctorsMap;
