...
public class PrescriptionService {
	...
	public ResponseEntity<Map<String, Object>> getPrescription(Long appointmentId) {
		...
		try {
			...
			if (scripts != null) {
				response.put("message", "Prescription found for appointment " + appointmentId);
				response.put("status", "200 OK");
				response.put("data", scripts);
				return ResponseEntity.status(HttpStatus.OK).body(response);
			}
			else {
				response.put("message", "Prescription not found for appointment " + appointmentId);
				response.put("status", "404 OK");
				response.put("data", scripts);
				return ResponseEntity.status(HttpStatus.NOT_FOUND).body(response);
			}
		}
		catch(Exception e) {
			response.put("message", "Error getting prescription for appointment " + appointmentId);
			response.put("status","500 Internal Server Error");
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
		}
	}
